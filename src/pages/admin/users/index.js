...
import { getUsers, addUser, updateUser, getUser, deleteUser, getRoles } from '../../../services/user';

export async function getServerSideProps({ req }) {
  const cookieData = cookie.parse(req.headers.cookie || '');
  const { users } = await getUsers(cookieData.token);
  const { roles } = await getRoles(cookieData.token);

  return {
    props: { info: cookieData, users, roles },
  }
}

export default function User({info, users, roles})
{
